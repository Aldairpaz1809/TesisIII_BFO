...
ecutwfc_values=(25 30 35 40)

# Lista de pseudopotenciales a probar
pseudo_sets=(
    "o_pbe_v1.2.uspp.F.UPF bi_pbe_v1.uspp.F.UPF fe_pbe_v1.5.uspp.F.UPF"
    "o_pbesol_v1.2.uspp.F.UPF bi_pbesol_v1.uspp.F.UPF fe_pbesol_v1.5.uspp.F.UPF"
)
...
# Iterar sobre los conjuntos de pseudopotenciales
for pseudo_set in "${pseudo_sets[@]}"; do
    ...
    for ecutwfc in "${ecutwfc_values[@]}"; do
        ...
        input_file="${name}_${pseudo_o}_ecutwfc_${ecutwfc}.in"
        output_file="${name}_${pseudo_o}_ecutwfc_${ecutwfc}.out"
...
EOF
        ...
        tiempo=$(grep 'PWSCF' $output_file | tail -1 | awk '{print $(NF-1), $NF}')

        # Extraer el número de iteraciones
        iteraciones=$(grep 'iteration #' $output_file | wc -l)

        # Guardar los resultados en el archivo .dat
        echo "$pseudo_o    $ecutwfc    $energia    $tiempo    $iteraciones" >> $output_dat_file

        # Mostrar resultado relevante
        echo "Resultado para $pseudo_o con ecutwfc = $ecutwfc:"
        grep '!' $output_file
