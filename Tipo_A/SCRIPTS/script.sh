...
ecutwfc=40 # Ry
kpoints="3 3 2 0 0 0"  # kpoints para SCF
kpoints_nscf=" 6 6 4 0 0 0"  # kpoints para NSCF
pseudo_set="o_pbe_v1.2.uspp.F.UPF bi_pbe_v1.uspp.F.UPF fe_pbe_v1.5.uspp.F.UPF"  # pseudo potenciales

# Directorios
base_dir="/home/aldair1809/Documentos/QE/4BFO/temp_tipoA"
...
n=6                   # Número de cores por defecto
...
epsilon() {
  local u_value=$1  # Recibe el valor de u_value como primer argumento
  local n=$2 # Recibe el valor de n como segundo argumento
  echo "Ejecutando cálculo eps  ecutwfc=$ecutwfc "
  # Llama al script scf.sh pasando los parámetros correspondientes
    chmod +x epsilon.sh
result=$(./epsilon.sh -e "${ecutwfc}" -k "${kpoints}" -b "$base_dir" -p "$pseudo_dir" -s "$pseudo_set" -u "$u_value" -n "$n")
...
show_help() {
  ...
  echo
  echo "Opciones:"
  echo "  -c    Cálculos a ejecutar (1: SCF, 2: Densidad de carga(requiere scf), 3: NSCF(requiere scf), 4: P-DOS(requiere nscf), 5: Bandas(requiere scf), 6: Post-procesamiento de bandas(requiere Bandas)), 7: eps (no requiere scf)"
